...
const EditProperty = () => {
        ...
        const fetchProperty = async () => {
            try {
                ...
                setFormData({
                    ...
                    description: p.description, tags: p.tags ? p.tags.join(', ') : '',
    ...
    const removeExistingImage = (indexToRemove) => {
        setFormData(prev => ({
            ...prev, galleryImages: prev.galleryImages.filter((_, idx) => idx !== indexToRemove)
        }));
    };
    ...
    return (
        <div style={{maxWidth:'800px', margin:'40px auto', background:'white', padding:'30px', borderRadius:'12px', boxShadow:'0 4px 15px rgba(0,0,0,0.05)'}}>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <h4 style={sectionHeader}>Furnishing</h4>
                <div className="form-group" style={{marginBottom:'15px'}}><label>Status</label><select name="furnishingStatus" value={formData.furnishingStatus} onChange={handleChange} className="input-field" style={{fontWeight:'bold'}}><option value="Unfurnished">Unfurnished</option><option value="Semi-Furnished">Semi-Furnished</option><option value="Fully Furnished">Fully Furnished</option></select></div>
                {formData.furnishingStatus !== 'Unfurnished' && (
                    <div className="checkbox-grid">{furnishingOptions.map(item => (<label key={item} className="checkbox-label"><input type="checkbox" value={item} checked={formData.furnishingItems.includes(item)} onChange={(e) => handleCheckbox(e, 'furnishingItems')} />{item}</label>))}</div>
                )}

                <h4 style={sectionHeader}>Amenities</h4>
                <div className="checkbox-grid">{amenityOptions.map(amenity => (<label key={amenity} className="checkbox-label"><input type="checkbox" value={amenity} checked={formData.amenities.includes(amenity)} onChange={(e) => handleCheckbox(e, 'amenities')} />{amenity}</label>))}</div>
                ...
                <div style={{marginBottom:'20px'}}>
                    ...
                    <div style={{display:'flex', gap:'15px', alignItems:'center'}}>
                        <img src={mainFile ? URL.createObjectURL(mainFile) : formData.mainImage} alt="Main" style={{width:'100px', height:'80px', objectFit:'cover', borderRadius:'6px', border:'1px solid #ddd'}} />
                        <div><p style={{fontSize:'0.85rem', color:'#666', margin:0}}>Change Cover Image:</p><input type="file" onChange={(e) => setMainFile(e.target.files[0])} /></div>
                    </div>
                </div>
                ...
                <div style={{marginBottom:'20px'}}>
                    <label style={{display:'block', fontWeight:'600', marginBottom:'5px'}}>Gallery Images</label>
                    <div style={{display:'flex', flexWrap:'wrap', gap:'10px', marginBottom:'10px'}}>
                        {formData.galleryImages.map((img, idx) => (<div key={idx} style={{position:'relative'}}><img src={img} alt="gallery" style={{width:'80px', height:'60px', objectFit:'cover', borderRadius:'4px'}} /><button type="button" onClick={() => removeExistingImage(idx)} style={{position:'absolute', top:'-5px', right:'-5px', background:'red', color:'white', borderRadius:'50%', border:'none', width:'20px', height:'20px', cursor:'pointer'}}>×</button></div>))}
                    </div>
                    <div style={{background:'#f8fafc', padding:'10px', borderRadius:'6px'}}><p style={{fontSize:'0.85rem', margin:'0 0 5px 0'}}>Add More Photos:</p><input type="file" multiple onChange={(e) => setGalleryFiles(Array.from(e.target.files))} /></div>
                </div>

                <button type="submit" disabled={uploading} className="btn" style={{width:'100%', padding:'15px', background:'#2563eb', color:'white', fontSize:'1.1rem', marginTop:'20px'}}>{uploading ? "Updating..." : "Save Changes"}</button>
            </form>
            <style>{`.grid-2 { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px; } .form-group { margin-bottom: 10px; } .form-group label { display: block; font-weight: 600; font-size: 0.9rem; margin-bottom: 5px; color: #334155; } .input-field { width: 100%; padding: 10px; border: 1px solid #cbd5e1; borderRadius: 6px; } .checkbox-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(140px, 1fr)); gap: 10px; margin-bottom: 20px; } .checkbox-label { display: flex; alignItems: center; gap: 8px; background: #f8fafc; padding: 8px; border-radius: 6px; border: 1px solid #e2e8f0; cursor: pointer; font-size: 0.9rem; } .btn { border: none; border-radius: 8px; font-weight: bold; cursor: pointer; } @media(max-width: 600px) { .grid-2 { grid-template-columns: 1fr; } }`}</style>
        </div>
    );
};
const sectionHeader = { margin: '25px 0 15px', color: '#475569', borderBottom: '1px solid #e2e8f0', paddingBottom: '8px', fontSize: '1.1rem', fontWeight: '700' };
